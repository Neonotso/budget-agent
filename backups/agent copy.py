...
class Assistant(Agent):
    ...
    def add_transaction(self, context: RunContext, date: str, description: str, amount: float, transaction_type: str, category: str = ""):
        """Adds a new transaction to the budget. Date must be in YYYY-MM-DD format. Transaction type must be 'Income' or 'Expense'. The category must be an existing category from the budget. If the category does not exist, ask the user to provide an existing category."""
        if category:
            ...
            if category.lower() not in existing_categories:
                return {"status": "error", "message": f"Category '{category}' does not exist. Please provide an existing category from: {', '.join(existing_categories_response['categories'])}."}
    ...
    @function_tool()
    def get_transactions(self, context: RunContext, date: str = None, description: str = None, amount: float = None, transaction_type: str = None, category: str = None):
        ...
        return self.budget_manager.get_all_transactions()
