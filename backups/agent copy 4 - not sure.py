...
class Assistant(Agent):
    ...
    async def add_transaction(self, context: RunContext, date: str, description: str, amount: float, transaction_type: str, category: str = ""):
        ...
        if category:
            ...
            if category.lower() not in existing_categories:
                relevant_categories = self._get_relevant_categories(description, existing_categories_response["categories"])
                if relevant_categories:
                    return {
                        "status": "error",
                        "message": f"Category '{category}' does not exist. Would you like to create it or choose from these existing categories: {', '.join(relevant_categories)}?"
                    }
                else:
                    return {
                        "status": "error",
    ...
    def _get_relevant_categories(self, description: str, all_categories: list) -> list:
        relevant_categories = []
        description_lower = description.lower()
        for cat in all_categories:
            cat_lower = cat.lower()
            # Simple keyword matching: check if any word in category is in description
            if any(word in description_lower for word in cat_lower.split()):
